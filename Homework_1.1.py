...
def validate_matrix(matrix):
    ...
    try:
        cols = len(matrix[0])
        return all(isinstance(row, list) and len(row) == cols for row in matrix)
    except Exception:
        return False


def create_single_matrix(name):
    try:
        rows = input_positive_int(f"Enter number of rows for {name}: ")
        columns = input_positive_int(f"Enter number of columns for {name}: ")
    except Exception:
    ...
    for r in range(rows):
        ...
        for c in range(columns):
            try:
                element = input_int(f"Enter the element at position ({r+1}, {c+1}) for {name}: ")
            except Exception:
...
def matrix_creation(n):
    ...
    for i in range(n):
        name = f"Matrix {i+1}"
        data = create_single_matrix(name)
        if data is None:
            return {}
        matrices[name] = data
    print(matrices)
...
def main():
    num_matrices = input_int("Enter the number of matrices to create: ")
